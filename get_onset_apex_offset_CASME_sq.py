...
subject_map = {
    ...
    "6": "s22", "7": "s23", "8": "s24", "9": "s25", "10": "s26",
...
emotion_to_filename_code = {
    "disgust1": "0101", "disgust2": "0102",
    "anger1": "0401", "anger2": "0402",
    "happy1": "0502", "happy2": "0503",
...
def get_CASME_sq_onset_apex_offset(src_root, dst_root, excel_path):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing CASME_sq"):
        # ===================== 读取每一列 =====================
        label_str = str(row[0]).strip()
        file_label = str(row[1]).strip()
        onset = safe_parse_int(row[2])
        apex = safe_parse_int(row[3])
        offset = safe_parse_int(row[4])
        expr_type = str(row[7]).strip()  # 第8列：表情类型

        # ===================== 核心过滤：只保留 micro-expression =====================
        if expr_type != "micro-expression":
            print(f"[SKIP] 过滤 macro: {file_label}")
            continue

        # 跳过无效帧 / 偏移帧为0
        if None in (onset, apex, offset) or offset == 0:
            print(f"[SKIP] 无效帧: {file_label}")
            continue

        # ===================== 映射 =====================
        subject = subject_map[label_str]
        subject_num = subject_num_map[label_str]
        ...
        for frame_type, frame_id in [('onset', onset), ('apex', apex), ('offset', offset)]:
            ...
            if os.path.exists(src_img_path):
                ...
                print(f"[WARNING] 不存在: {src_img_path}")
