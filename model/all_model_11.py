...
class SKD_TSTSAN(nn.Module):
    def __init__(self, out_channels=5, amp_factor=5):
        ...
        self.cross_attn1 = CrossBranchAttention(128)
        self.cross_attn2 = CrossBranchAttention(128)
        self.cross_attn3 = CrossBranchAttention(128)

        self.dropout = nn.Dropout(0.2)

        # 增加
        self.amp_predictor_L = nn.Sequential(
        ...
        self.amp_predictor_S = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(32, 1),
            nn.Sigmoid()
        ...
        self.amp_predictor_T = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(32, 1),
            nn.Sigmoid()
    ...
    def forward(self, input):
        ...
        # 修改
        amp1 = 1 + 2 * self.amp_predictor_L(motion_x1)
        amp1 = amp1.reshape(-1, 1, 1, 1)
        x1 = self.Aug_Manipulator_L(motion_x1_onset, motion_x1, amp1)
        ...
        # 修改
        amp2 = 1 + 2 * self.amp_predictor_S(motion_x2)
        amp2 = amp2.reshape(-1, 1, 1, 1)
        x2 = self.Aug_Manipulator_S(motion_x2_onset, motion_x2, amp2)
        ...
        # 修改
        amp3 = 1 + 2 * self.amp_predictor_T(motion_x3)
        amp3 = amp3.reshape(-1, 1, 1, 1)
        x3 = self.Aug_Manipulator_T(motion_x3_onset, motion_x3, amp3)
        ...
        AC1_x1_all = AC1_x1.reshape(AC1_x1.size(0), -1)
        ...
        AC1_x2_all = AC1_x2.reshape(AC1_x2.size(0), -1)
        ...
        AC1_x3_all = AC1_x3.reshape(AC1_x3.size(0), -1)

        # AC1_x3_all = AC1_x3_all.reshape((-1, 2) + AC1_x3_all.size()[1:])
        # AC1_x3_all = self.consensus(AC1_x3_all)
        # AC1_x3_all = AC1_x3_all.squeeze(1)
        # 修改
        AC1_x3_all = AC1_x3_all.reshape((-1, 2, AC1_x3_all.size(-1)))
        AC1_x3_all = self.temporal_attn(AC1_x3_all)
        # AC1_feature = torch.cat((AC1_x1_all, AC1_x2_all, AC1_x3_all), 1)
        # 修改
        AC1_x1_all, AC1_x2_all, AC1_x3_all = self.cross_attn1(
            AC1_x1_all, AC1_x2_all, AC1_x3_all
        )
        ...
        AC2_x1_all = AC2_x1.reshape(AC2_x1.size(0), -1)
        ...
        AC2_x2_all = AC2_x2.reshape(AC2_x2.size(0), -1)
        ...
        AC2_x3_all = AC2_x3.reshape(AC2_x3.size(0), -1)

        # AC2_x3_all = AC2_x3_all.reshape((-1, 2) + AC2_x3_all.size()[1:])
        # AC2_x3_all = self.consensus(AC2_x3_all)
        # AC2_x3_all = AC2_x3_all.squeeze(1)
        # 修改
        AC2_x3_all = AC2_x3_all.reshape((-1, 2, AC2_x3_all.size(-1)))
        AC2_x3_all = self.temporal_attn(AC2_x3_all)
        # AC2_feature = torch.cat((AC2_x1_all, AC2_x2_all, AC2_x3_all), 1)
        # 修改
        AC2_x1_all, AC2_x2_all, AC2_x3_all = self.cross_attn2(
            AC2_x1_all, AC2_x2_all, AC2_x3_all
        )
        ...
        x3_all = x3.reshape(x3.size(0), -1)

        # x3_all = x3_all.reshape((-1, 2) + x3_all.size()[1:])
        #
        # x3_all = self.consensus(x3_all)
        # x3_all = x3_all.squeeze(1)
        # 修改
        x3_all = x3_all.reshape((-1, 2, x3_all.size(-1)))
        x3_all = self.temporal_attn(x3_all)

        # final_feature = torch.cat((x1_all, x2_all, x3_all), 1)
        # 修改
        x1_all, x2_all, x3_all = self.cross_attn3(
            x1_all, x2_all, x3_all
        )
