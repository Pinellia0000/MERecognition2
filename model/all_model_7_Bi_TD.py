...
class TemporalDifference(nn.Module):
    def forward(self, x, n_segment):
        ...
        forward_pad = torch.zeros_like(forward_diff[:, :1])
        forward_diff = torch.cat([forward_pad, forward_diff], dim=1)

        # 反向差分
        backward_diff = x[:, :-1] - x[:, 1:]
        backward_pad = torch.zeros_like(backward_diff[:, :1])
        backward_diff = torch.cat([backward_diff, backward_pad], dim=1)

        # 融合
        out = x + 0.5 * forward_diff + 0.5 * backward_diff
