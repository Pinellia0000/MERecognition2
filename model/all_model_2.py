...
"""
保留新增的轻量空间注意力（CBAM-like）与 ECA，
并做了一个重要修复：
- 把 x3_onset 的 .cuda() 改为使用 input.device，避免 CPU 环境报错。
"""
...
class SKD_TSTSAN(nn.Module):
    def __init__(self, out_channels=5, amp_factor=5):
        ...
        self.AC1_conv1_T = TemporalShift(nn.Conv2d(64, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.AC1_conv2_T = TemporalShift(nn.Conv2d(128, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.conv2_T = TemporalShift(nn.Conv2d(64, out_channels=64, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.conv3_T = TemporalShift(nn.Conv2d(64, out_channels=64, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.AC2_conv1_T = TemporalShift(nn.Conv2d(64, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.AC2_conv2_T = TemporalShift(nn.Conv2d(128, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.conv4_T = TemporalShift(nn.Conv2d(64, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        self.conv5_T = TemporalShift(nn.Conv2d(128, out_channels=128, kernel_size=3, stride=1, padding=1), n_segment=2, n_div=8)
        ...
        # 更稳定的正则
        self.dropout_feat = nn.Dropout(0.3)
    ...
    def forward(self, input):
        ...
        # 第一阶段 conv1 + SA
        x1 = self.conv1_L(x1); x1 = self.bn1_L(x1); x1 = self.relu(x1); x1 = self.ECA1(x1); x1 = self.SA1(x1); x1 = self.maxpool(x1)
        x2 = self.conv1_S(x2); x2 = self.bn1_S(x2); x2 = self.relu(x2); x2 = self.SA2(x2); x2 = self.maxpool(x2)
        x3 = self.conv1_T(x3); x3 = self.bn1_T(x3); x3 = self.relu(x3); x3 = self.SA3(x3); x3 = self.maxpool(x3)

        # AC1 模块卷积 + SA
        AC1_x1 = self.AC1_conv1_L(x1); AC1_x1 = self.AC1_bn1_L(AC1_x1); AC1_x1 = self.relu(AC1_x1); AC1_x1 = self.AC1_ECA1(AC1_x1); AC1_x1 = self.SA4(AC1_x1)
        AC1_x1 = self.AC1_conv2_L(AC1_x1); AC1_x1 = self.AC1_bn2_L(AC1_x1); AC1_x1 = self.relu(AC1_x1); AC1_x1 = self.AC1_ECA2(AC1_x1); AC1_x1 = self.SA5(AC1_x1)
        AC1_x1 = self.AC1_pool(AC1_x1); AC1_x1_all = AC1_x1.view(AC1_x1.size(0), -1)

        AC1_x2 = self.AC1_conv1_S(x2); AC1_x2 = self.AC1_bn1_S(AC1_x2); AC1_x2 = self.relu(AC1_x2); AC1_x2 = self.SA6(AC1_x2)
        AC1_x2 = self.AC1_conv2_S(AC1_x2); AC1_x2 = self.AC1_bn2_S(AC1_x2); AC1_x2 = self.relu(AC1_x2); AC1_x2 = self.SA7(AC1_x2)
        AC1_x2 = self.AC1_pool(AC1_x2); AC1_x2_all = AC1_x2.view(AC1_x2.size(0), -1)

        AC1_x3 = self.AC1_conv1_T(x3); AC1_x3 = self.AC1_bn1_T(AC1_x3); AC1_x3 = self.relu(AC1_x3); AC1_x3 = self.SA8(AC1_x3)
        AC1_x3 = self.AC1_conv2_T(AC1_x3); AC1_x3 = self.AC1_bn2_T(AC1_x3); AC1_x3 = self.relu(AC1_x3); AC1_x3 = self.SA9(AC1_x3)
        AC1_x3 = self.AC1_pool(AC1_x3); AC1_x3_all = AC1_x3.view(AC1_x3.size(0), -1)

        AC1_x3_all = AC1_x3_all.view((-1, 2) + AC1_x3_all.size()[1:])
        AC1_x3_all = self.consensus(AC1_x3_all).squeeze(1)
        ...
        # 第二阶段高级卷积 x1/x2/x3
        x1 = self.conv2_L(x1); x1 = self.bn2_L(x1); x1 = self.relu(x1); x1 = self.ECA2(x1)
        x1 = self.conv3_L(x1); x1 = self.bn3_L(x1); x1 = self.relu(x1); x1 = self.ECA3(x1); x1 = self.SA10(x1)
        x1 = self.avgpool(x1)

        x2 = self.conv2_S(x2); x2 = self.bn2_S(x2); x2 = self.relu(x2)
        x2 = self.conv3_S(x2); x2 = self.bn3_S(x2); x2 = self.relu(x2)
        x2 = self.avgpool(x2)

        x3 = self.conv2_T(x3); x3 = self.bn2_T(x3); x3 = self.relu(x3)
        x3 = self.conv3_T(x3); x3 = self.bn3_T(x3); x3 = self.relu(x3)
        x3 = self.avgpool(x3)

        # AC2
        AC2_x1 = self.AC2_conv1_L(x1); AC2_x1 = self.AC2_bn1_L(AC2_x1); AC2_x1 = self.relu(AC2_x1); AC2_x1 = self.AC2_ECA1(AC2_x1)
        AC2_x1 = self.AC2_conv2_L(AC2_x1); AC2_x1 = self.AC2_bn2_L(AC2_x1); AC2_x1 = self.relu(AC2_x1); AC2_x1 = self.AC2_ECA2(AC2_x1)
        AC2_x1 = self.AC2_pool(AC2_x1); AC2_x1_all = AC2_x1.view(AC2_x1.size(0), -1)

        AC2_x2 = self.AC2_conv1_S(x2); AC2_x2 = self.AC2_bn1_S(AC2_x2); AC2_x2 = self.relu(AC2_x2)
        AC2_x2 = self.AC2_conv2_S(AC2_x2); AC2_x2 = self.AC2_bn2_S(AC2_x2); AC2_x2 = self.relu(AC2_x2)
        AC2_x2 = self.AC2_pool(AC2_x2); AC2_x2_all = AC2_x2.view(AC2_x2.size(0), -1)

        AC2_x3 = self.AC2_conv1_T(x3); AC2_x3 = self.AC2_bn1_T(AC2_x3); AC2_x3 = self.relu(AC2_x3)
        AC2_x3 = self.AC2_conv2_T(AC2_x3); AC2_x3 = self.AC2_bn2_T(AC2_x3); AC2_x3 = self.relu(AC2_x3)
        ...
        # Final
        x1 = self.conv4_L(x1); x1 = self.bn4_L(x1); x1 = self.relu(x1); x1 = self.ECA4(x1)
        x1 = self.conv5_L(x1); x1 = self.bn5_L(x1); x1 = self.relu(x1); x1 = self.ECA5(x1)
        x1 = self.all_avgpool(x1); x1_all = x1.view(x1.size(0), -1)

        x2 = self.conv4_S(x2); x2 = self.bn4_S(x2); x2 = self.relu(x2)
        x2 = self.conv5_S(x2); x2 = self.bn5_S(x2); x2 = self.relu(x2)
        x2 = self.all_avgpool(x2); x2_all = x2.view(x2.size(0), -1)

        x3 = self.conv4_T(x3); x3 = self.bn4_T(x3); x3 = self.relu(x3)
        x3 = self.conv5_T(x3); x3 = self.bn5_T(x3); x3 = self.relu(x3)
        x3 = self.all_avgpool(x3); x3_all = x3.view(x3.size(0), -1)

        x3_all = x3_all.view((-1, 2) + x3_all.size()[1:])
        x3_all = self.consensus(x3_all).squeeze(1)

        final_feature = torch.cat((x1_all, x2_all, x3_all), 1)
        x_all = self.dropout_feat(final_feature)
