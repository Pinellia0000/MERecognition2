...
# 注意修改
from model.all_model_13 import *

all_model_path = "/kaggle/working/MERecognition2/model/all_model_13.py"
"""
在fuctions_4的基础上
...
def plot_confusion_matrix(cm, class_names, dataset_name, save_dir):
    """
    绘制混淆矩阵
    根据 dataset_name 自动命名
    """
    # ===== 1. 创建目录 =====
    os.makedirs(save_dir, exist_ok=True)

    # ===== 2. 文件名 =====
    save_path = os.path.join(save_dir, f"{dataset_name}_cm.png")
    ...
    plt.ylabel("True")
    plt.title(dataset_name)  # 标题也用数据集名
...
def main_SKD_TSTSAN_with_Aug_with_SKD(config):
    ...
    elif dataset_name == "CASME2_retinaface_loso_5":
        class_names = ["happiness", "surprise", "disgust", "repression", "others"]
    elif dataset_name == "CASME3_retinaface_loso_7":
        class_names = ["happy", "surprise", "disgust", "anger", "fear", "sad", "others"]
    elif dataset_name == "CASME3_retinaface_loso_4":
        class_names = ["positive", "negative", "surprise", "others"]

    cm = confusion_matrix(total_gt, best_total_pred,
                          labels=list(range(len(class_names))))

    save_dir = f"/kaggle/working/MERecognition2/confusion_matrix_plot"

    plot_confusion_matrix(cm, class_names, dataset_name, save_dir)
    UF1, UAR = recognition_evaluation(dataset_name, total_gt, total_pred, show=True)
    best_UF1, best_UAR = recognition_evaluation(dataset_name, total_gt,
                                                best_total_pred, show=True)
    print('UF1:', round(UF1, 4), '| UAR:', round(UAR, 4))
